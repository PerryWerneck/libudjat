	...
	namespace Udjat {
		...
		namespace Factory {
			...
			void UDJAT_API insert(const char *name, std::function<std::shared_ptr<Abstract::Agent>(Abstract::Agent &parent, const pugi::xml_node &node)> factory);

			/// @brief Register a node factory.
			void UDJAT_API insert(const char *name, std::function<void(std::shared_ptr<Abstract::Agent> agent, const pugi::xml_node &node)> factory);
