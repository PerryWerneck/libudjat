...
#pragma once
 ...
 namespace Udjat {
	...
	public:
		...
		virtual std::shared_ptr<Abstract::Agent> AgentFactory(const Abstract::Object &parent, const pugi::xml_node &node);

		/// @brief Build alert from XML node, used on tag <alert type='module-name'>
		virtual std::shared_ptr<Abstract::Alert> AlertFactory(const Abstract::Object &parent, const pugi::xml_node &node);
