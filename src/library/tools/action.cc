 ...
 */
 ...
 namespace Udjat {
	...
	void Action::load(const XML::Node &node, std::vector<std::shared_ptr<Action>> &actions) {

		if(!String{node,"action-type"}.empty()) {
			actions.push_back(Action::Factory::build(node,"action-type",true));
			return;
		}

		for(const char *nodename : { "action", "script" }) {
			for(auto action = node.child(nodename); action; action = action.next_sibling(nodename)) {
				actions.push_back(Action::Factory::build(node,nodename,true));
			}
		}
